<template>
  <div class="dashboard">
    ...
    <div class="header">
      <div class="title-box">
        <h1>🗽 纽约出租车数据驾驶舱</h1>
        <p>2018年全年 · NYC TLC开放数据 · 黄色出租车 vs 绿色出租车</p>
      </div>
      <div class="date-box">
    ...
    <div class="kpi-grid">
      <div class="kpi-card" v-for="(item, idx) in kpiList" :key="idx">
        ...
        <div class="kpi-trend" :class="item.trend >= 0 ? 'trend-up' : 'trend-down'">
    ...
    <div class="filter-toolbar">
      ...
      <div class="filter-note">※ 筛选后仅KPI数值变化，图表展示全量数据（联动演示）</div>
    </div>
    ...
    <div v-if="loading" class="loader-overlay">
      ...
      <div class="loader-text">正在加载2018年全年数据，请稍候...</div>
    </div>
  </div>
</template>
...
// KPI数据
const kpiList = ref([
  { icon: '🚕', label: '总订单量', value: 0, unit: '单', trend: 12.5 },
  { icon: '💰', label: '总营收', value: 0, unit: '万美元', trend: 8.3 },
  { icon: '💝', label: '平均小费率', value: 0, unit: '%', trend: 5.2 },
  { icon: '📏', label: '平均行程距离', value: 0, unit: '英里', trend: -2.1 },
  { icon: '💵', label: '平均费用', value: 0, unit: '$', trend: 3.7 },
  { icon: '⏰', label: '晚高峰占比', value: 0, unit: '%', trend: 6.8 }
])
...
const applyFilters = async () => {
  try {
    const params = {}
    if (filters.company) params.车辆类型 = filters.company
    if (filters.borough) params.行政区 = filters.borough
    if (filters.minFare !== null && filters.minFare !== undefined && filters.minFare !== '') params.最小费用 = filters.minFare
    if (filters.maxFare !== null && filters.maxFare !== undefined && filters.maxFare !== '') params.最大费用 = filters.maxFare

    const res = await axios.get('http://127.0.0.1:8000/api/filter-data', { params })
    const filterData = res.data
    // 更新KPI中的总订单量和平均费用、平均距离
    kpiList.value[0].value = filterData.筛选后记录数 || 0
    kpiList.value[4].value = filterData.平均费用 || 0
    kpiList.value[3].value = filterData.平均距离 || 0
  } catch (err) {
