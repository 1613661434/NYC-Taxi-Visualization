<template>
  <ChartCard title="出行时段分布" subtitle="早高峰/白天/晚高峰/夜间/深夜">
...
const doRender = () => {
  const period = props.data?.period_dist || {}
  ...
  chart.setOption({
    ...
    series: [{
      ...
      data: Object.entries(period).map(([n,v],i) => ({ name: n, value: v, itemStyle: { color: colors[i%colors.length] } })),
    }],
  })
}
